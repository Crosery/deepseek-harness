...
Tool calls render like the web, framed like omp: a live braille activity line while they run — the label truncated to one row, never wrapping — (one dim pending row per call in piped runs), then a bordered card: the tool name on the dim top border, a `✓`/`✗` status row with the flattened one-line label and duration, and the dimmed render-intent preview inside the box (terminal text, diff hunks, search matches, read content, web output), with nested subcalls indented within the same frame. Command outcomes print under their `⌘` line as `✓`/`✗` rows.
